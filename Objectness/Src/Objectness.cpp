...
{
	if (modelName.size() == 0) modelName = _modelName;
	...
	if (!matRead(s1, filters1f) || !matRead(sI, idx1i)){
		//cout << "... [ Can't load model: " << s1 << " or " << sI << endl;
		return 0;
	}
	...
	if (!matRead(s2, _svmReW1f) || _svmReW1f.size() != Size(2, _svmSzIdxs.size())){
		_svmReW1f = Mat();
		//cout << "... [ Can't load model: " << s2 << endl;
		return -1;
	}
	cout << "... [ Success to load models: " << s1 << " and " << s2 << endl;
	return 1;
}
...
void Objectness::predictBBoxSI(CMat &img3u, ValStructVec<float, Vec4i> &valBoxes, vecI &sz, int NUM_WIN_PSZ, bool fast)
{
    ...
    //std::cout << _voc.localDir + format("%d.png", r) << std::endl;
...
#pragma omp parallel for
		...
		tm.Stop();
		cout << "... [ Average time for predicting an image " <<  _clrName[_Clr] << " is" << tm.TimeInSeconds()/TestNum << "s" << endl;
	}
...
#pragma omp parallel for
	for (int i = 0; i < TestNum; i++)
  {
	...
	tm.Stop();
  cout << "--- [ All images are Predicted" << endl;
  cout << "--- [ Average time for predicting an image (" << _clrName[_Clr] << ") is " << tm.TimeInSeconds()/TestNum << "s" << endl;
